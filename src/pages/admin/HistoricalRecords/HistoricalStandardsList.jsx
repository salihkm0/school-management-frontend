...
const HistoricalStandardsList = () => {
  ...
  useEffect(() => {
    const fetchStandards = async () => {
      try {
        const res = await historicalImportService.getHierarchicalStandards(year)
        if (res.data?.success) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-1">Select a Standard</p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        {standards.map((std) => (
          ...
          >
            ...
            <span className="text-sm font-medium text-gray-900">Standard {std.item || std}</span>
            {std.count !== undefined && (
      ...
      {standards.length === 0 && (
        <div className="text-center py-12 bg-white rounded-xl border border-gray-200 border-dashed">
          ...
          <h3 className="mt-2 text-sm font-semibold text-gray-900">No standards found</h3>
          <p className="mt-1 text-sm text-gray-500">No records exist for this academic year.</p>
        </div>
      )}
    </div>
  )
...
export default HistoricalStandardsList
